...
from .attention import fusion block
...
class LiNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_attn1 = fusion_block(filters[0])
        self.self_attn2 = fusion_block(filters[1])
        self.self_attn3 = fusion_block(filters[2])
        self.self_attn4 = fusion_block(filters[3])
        self.self_attn5 = fusion_block(filters[4])
...
if __name__ == "__main__":
    # 模拟输入 (batch_size=1, channel=1, H=256, W=256)
    art = torch.randn(1, 1, 224, 224)
    pv = torch.randn(1, 1, 224, 224)
    delay = torch.randn(1, 1, 224, 224)

    # 初始化模型
    args = None  # 如果你的 LiNet 不依赖 args，可以传 None
    model = LiNet(args)

    # 把三个输入封装成一个 tuple，方便 thop 统计
    inputs = (art, pv, delay)
