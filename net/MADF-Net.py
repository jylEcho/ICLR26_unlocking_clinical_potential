...
from attentionModule import fusion
...
class conv_block(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        # lite:
        # self.conv = nn.Sequential(
        #     nn.Conv2d(in_ch, in_ch, kernel_size=3, stride=1, padding=1, groups=in_ch, bias=True),  # 深度卷积
        #     nn.Conv2d(in_ch, out_ch, kernel_size=1, stride=1, padding=0, bias=True),  # 逐点卷积（1x1）
        #     nn.BatchNorm2d(out_ch),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(out_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=out_ch, bias=True),  # 深度卷积
        #     nn.Conv2d(out_ch, out_ch, kernel_size=1, stride=1, padding=0, bias=True),  # 逐点卷积（1x1）
        #     nn.BatchNorm2d(out_ch),
        #     nn.ReLU(inplace=True)
        #     )
...
class LiNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.self_attn1 = fusion(filters[0])
        self.self_attn2 = fusion(filters[1])
        self.self_attn3 = fusion(filters[2])
        self.self_attn4 = fusion(filters[3])
        self.self_attn5 = fusion(filters[4])
